...
async function getTestUserIds(): Promise<
  Array<{ email: string; userId: string }>
> {
  const emails = Array.from(
    { length: 100 },
    (_, i) => `test${i + 26}@readygo.test`
  );
...
async function main() {
  ...
  try {
    ...
    const mappedUsers = testUsers
      .slice(0, STEAM_IDS.length)
      .map((user, index) => ({
        userId: user.userId,
    ...
    const response = await fetch(
      `${DEV_SERVER_URL}/api/admin/steam/bulk-link`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (result.summary.failure > 0) {
      ...
      result.results
        ...
        .forEach((r: any, index: number) => {
          console.log(
            `  ${index + 1}. ${r.userId}: ${r.error} (${r.errorCode})`
          );
        });
    }
  } catch (error) {
